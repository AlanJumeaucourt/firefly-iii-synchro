...
class DiscordBot:
    def __init__(self, token: str, channel_id: int, firefly_api: FireflyIIIAPI):
        ...
        intents.messages = (
            True  # Enable messages intent, necessary to read message content
        )
    ...
    async def on_reaction_add(self, reaction: Reaction, user: User):
        ...
        logger.info(
            f"Reaction {reaction.emoji} added by {user.name} on message {reaction.message.content}."
        )
        if (
            user != self.bot.user
            and reaction.emoji == "➕"
            and reaction.message.author == self.bot.user
            and "🔄" not in [r.emoji for r in reaction.message.reactions]
        ):
            ...
            if transaction_sha256:
                ...
                if transaction:
                    ...
                    new_embed = discord.Embed.from_dict(
                        reaction.message.embeds[0].to_dict()
                    )
                    ...
                    logger.error(
                        f"Failed to find transaction with sha256 {transaction_sha256}"
                    )
    ...
    async def post_missing_transactions(self):
        ...
        if self.missing_transactions:
            for transaction in self.missing_transactions:
                transaction_exists = await self.is_transaction_posted(
    ...
    async def is_transaction_posted(
        self, channel: TextChannel, transaction_sha256: str
    ):
    ...
    def format_transaction_embeded(self, transaction: Transaction) -> Embed:
        ...
        embed.add_field(
            name="Sha256", value=self.sha256_transaction(transaction), inline=False
        )
